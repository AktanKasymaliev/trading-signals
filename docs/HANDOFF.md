...
- **Analysis-assistant AI mode landed (2026-05-15).** Added `xau_pro_bot/models/ai_explanation.py` (model_name / derive_action / derive_risk_label / short_reason). Engine now emits `ai_model_name`, `ai_feature_set`, `ai_action` (KEEP/BLOCK/DOWNGRADE), `ai_reason_short`, `ai_risk_label` (CLEAN_SETUP/MEDIUM_RISK/HIGH_RISK), and `ai_pre_block_tier`. New env `AI_EXPLAIN=true` switches the Telegram formatter from the legacy single-line AI output to a compact 4-line block ("🧠 AI filter / Модель / Риск / Причина"). Backtester now records `blocked_details` per blocked signal. Tests: `tests/test_ai_explain.py` (32 cases). Suite green: 296 passed, 3 skipped. **Path C legacy remains the production model**; Path F stationary stays research-only.
